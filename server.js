...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'SoleVault',time:new Date().toISOString()}));
...
app.post('/api/products',(req,res)=>{const {name,category,gender='Unisex',price,rating=5,image,description='',stock=0,sizes='6,7,8,9,10,11,12'}=req.body;const n=String(name||'').trim(),c=String(category||'').trim(),img=String(image||'').trim(),pr=Number(price),rt=Number(rating),st=Number(stock);if(!n||!c||!img||!Number.isFinite(pr)||pr<=0)return res.status(400).json({error:'Valid name, category, price and image are required'});if(!Number.isFinite(rt)||rt<0||rt>5)return res.status(400).json({error:'Rating must be between 0 and 5'});if(!Number.isInteger(st)||st<0)return res.status(400).json({error:'Stock must be a non-negative integer'});const info=db.prepare('INSERT INTO products(name,category,gender,price,rating,image,description,stock,sizes) VALUES(?,?,?,?,?,?,?,?,?)').run(n,c,gender,pr,rt,img,String(description||''),st,String(sizes||'6,7,8,9,10,11,12'));res.status(201).json(db.prepare('SELECT * FROM products WHERE id=?').get(info.lastInsertRowid))});
app.put('/api/products/:id',(req,res)=>{const old=db.prepare('SELECT * FROM products WHERE id=?').get(req.params.id);if(!old)return res.status(404).json({error:'Product not found'});const p={...old,...req.body},pr=Number(p.price),rt=Number(p.rating),st=Number(p.stock);if(!String(p.name||'').trim()||!String(p.category||'').trim()||!String(p.image||'').trim()||!Number.isFinite(pr)||pr<=0)return res.status(400).json({error:'Valid name, category, price and image are required'});if(!Number.isFinite(rt)||rt<0||rt>5)return res.status(400).json({error:'Rating must be between 0 and 5'});if(!Number.isInteger(st)||st<0)return res.status(400).json({error:'Stock must be a non-negative integer'});db.prepare('UPDATE products SET name=?,category=?,gender=?,price=?,rating=?,image=?,description=?,stock=?,sizes=? WHERE id=?').run(String(p.name).trim(),String(p.category).trim(),p.gender||'Unisex',pr,rt,String(p.image).trim(),String(p.description||''),st,String(p.sizes||'6,7,8,9,10,11,12'),req.params.id);res.json(db.prepare('SELECT * FROM products WHERE id=?').get(req.params.id))});
...
app.post('/api/orders',(req,res)=>{const {customer_name,email,phone='',address,items}=req.body;if(!String(customer_name||'').trim()||!String(email||'').trim()||!String(address||'').trim()||!Array.isArray(items)||!items.length)return res.status(400).json({error:'Missing order information'});try{const orderId=db.transaction(()=>{let total=0;const resolved=[];for(const i of items){const p=db.prepare('SELECT * FROM products WHERE id=?').get(i.product_id);if(!p)throw new Error('Product not found');const qty=Number(i.qty),sz=Number(i.size);if(!Number.isInteger(qty)||qty<1)throw new Error('Invalid quantity');if(!Number.isInteger(sz)||!String(p.sizes).split(',').map(Number).includes(sz))throw new Error(`Size ${i.size} is unavailable for ${p.name}`);if(p.stock<qty)throw new Error(`Insufficient stock for ${p.name}`);total+=p.price*qty;resolved.push({p,i,qty,sz})}const order=db.prepare('INSERT INTO orders(customer_name,email,phone,address,total) VALUES(?,?,?,?,?)').run(String(customer_name).trim(),String(email).trim(),String(phone||'').trim(),String(address).trim(),total);const addItem=db.prepare('INSERT INTO order_items(order_id,product_id,size,qty,price) VALUES(?,?,?,?,?)');for(const x of resolved){addItem.run(order.lastInsertRowid,x.p.id,x.sz,x.qty,x.p.price);db.prepare('UPDATE products SET stock=stock-? WHERE id=?').run(x.qty,x.p.id)}return order.lastInsertRowid})() ;res.status(201).json({order_id:orderId})}catch(e){res.status(400).json({error:e.message})}});
